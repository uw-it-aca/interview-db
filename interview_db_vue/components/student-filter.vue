// student-filter.vue // to filter student interviews

<template>
  <div
    class="card p-3 border-0 filter-card"
    :class="mq.tablet || mq.mobile ? 'card-100 border-10' : 'card'"
  >
    <div v-if="!mq.tablet && !mq.mobile" class="row m-3">
      <div class="justify-content-end d-flex">
        <a
          v-if="!emptyFilters"
          class="text-secondary active-link active-link-hover"
          @click="clearFilters"
        >
          Clear All
        </a>
    ...
    <div class="card-body">
      <div class="row mb-4">
        <h2
          class="d-flex fw-bold display-6 text-purple justify-content-start"
          :class="mq.tablet || mq.mobile ? 'col-9' : ''"
        >
          Filter Stories
        </h2>
        <div
          v-if="mq.tablet || mq.mobile"
          class="d-flex col-3 justify-content-end p-0"
        >
          <button
            v-if="topicId == undefined"
            type="button"
            class="btn-close"
            aria-label="Close"
            @click="
      ...
      <div class="mb-5">
        <p class="display-4 text-purple fw-bold fs-5 mb-3">Student Year</p>
        <div class="mt-0" id="year">
          <div class="justify-content-start col-12">
            <input
              ...
              @change="updateQuery()"
            />
            <label class="filter-chip" for="Freshman"> Freshman </label>
            <input
              type="checkbox"
              class="btn-check"
              id="Sophomore"
              ...
              @change="updateQuery()"
            />
            <label class="filter-chip" for="Sophomore"> Sophomore </label>
            <input
              type="checkbox"
              class="btn-check"
              id="Junior"
              ...
              @change="updateQuery()"
            />
            <label class="filter-chip" for="Junior"> Junior </label>
            <input
              type="checkbox"
              class="btn-check"
              id="Senior"
              ...
              @change="updateQuery()"
            />
            <label class="filter-chip" for="Senior"> Senior + </label>
          </div>
        </div>
      </div>

      <div class="mb-5">
        <p class="display-4 fs-5 fw-bold text-purple mb-3">Major</p>
      ...
      <div v-if="story || topicId != undefined" class="mb-5"></div>
      <div v-else class="mb-5">
        <p class="display-4 fs-5 text-purple fw-bold mb-3">Story Collection</p>
      ...
      <div>
        <div v-if="mq.mobile || mq.tablet" class="row fixed-bottom">
          ...
          <button
            v-if="topicId == undefined"
            class="btn btn-gold btn-block fw-bold w-50"
            @click="
              $router.push({
                name: 'Students',
                query: { ...this.$route.query },
  ...
  computed: {
    emptyFilters() {
      return (
        ...
        this.filters.major.length == 0 &&
        this.filters.topic.length == 0
      );
    },
  },
};
